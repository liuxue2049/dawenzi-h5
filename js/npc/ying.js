...
const PROPS = {
  1: { speed: 1.2, maxHealth: 90, attackDamage: 10, attackInterval: 3000, attack: true },
  2: { speed: 1.0, maxHealth: 360, attackDamage: 18, attackInterval: 5000, attack: true },
  3: { speed: 1.5, maxHealth: 100, attackDamage: 8, attackInterval: 2800, attack: true, health: true, currentHealth: 100 },
  4: { speed: 0.8, maxHealth: 120, attackDamage: 12, attackInterval: 4000, attack: true, heal: true },
  5: { speed: 1.8, maxHealth: 60, attackDamage: 10, attackInterval: 3000, attack: true, stealth: true },
};
const SF = 60, SC = 8, SFP = 10, FW = 64, FH = 64;
const MT = 70, MBO = 100, MS = 10;

export default class Ying extends Sprite {
  constructor(type = 1) {
    super();
  ...
  render(ctx) {
    ...
    else { ctx.fillStyle = '#FFD700'; ctx.fillRect(this.x, this.y, this.width, this.height); ctx.fillStyle = '#000'; ctx.font = '10px Arial'; ctx.textAlign = 'center'; ctx.fillText(`Y${this.mosquitoType}`, this.x + this.width / 2, this.y + this.height / 2 + 4); }
