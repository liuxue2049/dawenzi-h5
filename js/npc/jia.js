...
const PROPS = {
  ...
  2: { speed: 0.9, maxHealth: 450, attackDamage: 20, attackInterval: 5000, attack: true },
  3: { speed: 1.4, maxHealth: 150, attackDamage: 10, attackInterval: 3000, attack: true, health: true, currentHealth: 150 },
  4: { speed: 0.8, maxHealth: 160, attackDamage: 14, attackInterval: 4500, attack: true, heal: true },
  5: { speed: 1.7, maxHealth: 80, attackDamage: 10, attackInterval: 3000, attack: true, stealth: true },
};
const SF = 60, SC = 8, SFP = 10, FW = 64, FH = 64;
const MT = 70, MBO = 100, MS = 10;

export default class Jia extends Sprite {
  constructor(type = 1) {
    super();
    this.mosquitoType = type; this.id = type; this.width = 50; this.height = 50;
  ...
  render(ctx) {
    ...
    else { ctx.fillStyle = '#2E8B57'; ctx.fillRect(this.x, this.y, this.width, this.height); ctx.fillStyle = '#fff'; ctx.font = '10px Arial'; ctx.textAlign = 'center'; ctx.fillText(`J${this.mosquitoType}`, this.x + this.width / 2, this.y + this.height / 2 + 4); }
