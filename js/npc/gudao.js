...
const PROPS = {
  1: { speed: 1.3, maxHealth: 100, attackDamage: 10, attackInterval: 3000, attack: true },
  2: { speed: 1.0, maxHealth: 420, attackDamage: 20, attackInterval: 5000, attack: true },
  ...
  5: { speed: 1.8, maxHealth: 70, attackDamage: 10, attackInterval: 3000, attack: true, stealth: true },
};
const SF = 60, SC = 8, SFP = 10, FW = 64, FH = 64;
const MT = 70, MBO = 100, MS = 10;

export default class Gudao extends Sprite {
  constructor(type = 1) {
    super();
    this.mosquitoType = type; this.id = type; this.width = 48; this.height = 48;
  ...
  render(ctx) {
    ...
    else { ctx.fillStyle = '#32CD32'; ctx.fillRect(this.x, this.y, this.width, this.height); ctx.fillStyle = '#fff'; ctx.font = '10px Arial'; ctx.textAlign = 'center'; ctx.fillText(`G${this.mosquitoType}`, this.x + this.width / 2, this.y + this.height / 2 + 4); }
