...
const PROPS = {
  ...
  4: { speed: 1.0, maxHealth: 110, attackDamage: 12, attackInterval: 4000, attack: true, heal: true },
  5: { speed: 2.0, maxHealth: 55, attackDamage: 10, attackInterval: 2800, attack: true, stealth: true },
};
const SF = 60, SC = 8, SFP = 12, FW = 64, FH = 64;
const MT = 70, MBO = 100, MS = 10;

export default class Ding extends Sprite {
  constructor(type = 1) {
    super();
  ...
  render(ctx) {
    ...
    else { ctx.fillStyle = '#00CED1'; ctx.fillRect(this.x, this.y, this.width, this.height); ctx.fillStyle = '#fff'; ctx.font = '10px Arial'; ctx.textAlign = 'center'; ctx.fillText(`D${this.mosquitoType}`, this.x + this.width / 2, this.y + this.height / 2 + 4); }
