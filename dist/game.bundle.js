(() => {
  ...
  var Animation = class {
    ...
    drawToCanvas(ctx3, x, y, w, h, opacity = 1) {
  ...
  var Enemy = class extends Sprite {
    ...
    render(ctx3) {
      ...
      if (this.imgLoaded && this.anim) {
        this.anim.drawToCanvas(ctx3, this.x, this.y, this.width, this.height, this.opacity);
      } else {
  ...
  var UFO = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FRAME_W2, FRAME_H2, SPRITE_COLS2, SPRITE_FRAMES2, SPRITE_FPS2);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FRAME_W2, FRAME_H2, SPRITE_COLS2, SPRITE_FRAMES2, SPRITE_FPS2);
        }).catch(() => {
        });
      }
  ...
  var Fjg = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FRAME_W3, FRAME_H3, SPRITE_COLS3, SPRITE_FRAMES3, SPRITE_FPS3);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FRAME_W3, FRAME_H3, SPRITE_COLS3, SPRITE_FRAMES3, SPRITE_FPS3);
        }).catch(() => {
        });
      }
  ...
  var Jia = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FW, FH, SC, SF, SFP);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FW, FH, SC, SF, SFP);
        }).catch(() => {
        });
      }
  ...
  var Piao = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FW2, FH2, SC2, SF2, SFP2);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FW2, FH2, SC2, SF2, SFP2);
        }).catch(() => {
        });
      }
  ...
  var Gudao = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FW3, FH3, SC3, SF3, SFP3);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FW3, FH3, SC3, SF3, SFP3);
        }).catch(() => {
        });
      }
  ...
  var Ying = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FW4, FH4, SC4, SF4, SFP4);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FW4, FH4, SC4, SF4, SFP4);
        }).catch(() => {
        });
      }
  ...
  var Ding = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FW5, FH5, SC5, SF5, SFP5);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FW5, FH5, SC5, SF5, SFP5);
        }).catch(() => {
        });
      }
  ...
  var Die = class extends Sprite {
    ...
    init() {
      ...
      if (this.img) {
        this.imgLoaded = true;
        this.anim = new Animation(this.img, FW6, FH6, SC6, SF6, SFP6);
      } else {
        resloader_default.loadImage(src).then((img) => {
          this.img = img;
          this.imgLoaded = true;
          this.anim = new Animation(img, FW6, FH6, SC6, SF6, SFP6);
        }).catch(() => {
        });
      }
  ...
  var GameInfo = class extends TinyEmitter {
    ...
    _renderBottleSelect(ctx3) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        for (let x = 0; x <= SCREEN_WIDTH; x += 10) {
          const y = SCREEN_HEIGHT * 0.3 + i * 80 + Math.sin(x * 0.01 + this._oceanOff + i) * 20;
          x === 0 ? ctx3.moveTo(x, y) : ctx3.lineTo(x, y);
        }
        ctx3.stroke();
      }
      ctx3.fillStyle = "#FFF";
      ctx3.font = "bold 26px Arial";
      ctx3.textAlign = "center";
      ctx3.fillText("\u{1F30A} \u9009\u62E9\u751F\u6001\u74F6", SCREEN_WIDTH / 2, 50);
      ...
      Object.entries(BOTTLE_CONFIG).forEach(([id, cfg], idx) => {
        ...
        if (y + ch < 0 || y > SCREEN_HEIGHT) return;
        ctx3.fillStyle = "rgba(255,255,255,0.15)";
        ctx3.beginPath();
        ctx3.roundRect(x, y, cw, ch, 10);
        ctx3.fill();
        ctx3.strokeStyle = "rgba(255,255,255,0.3)";
        ctx3.lineWidth = 1;
        ctx3.stroke();
        const img = resloader_default.getImage(cfg.icon);
        if (img) {
          const sz = Math.min(cw * 0.5, ch * 0.4);
          ctx3.drawImage(img, x + (cw - sz) / 2, y + ch * 0.05, sz, sz);
        } else {
          ctx3.fillStyle = "#FFF";
          ctx3.font = `${cw * 0.3}px Arial`;
          ctx3.fillText("?", x + cw / 2, y + ch * 0.35);
        }
        ctx3.fillStyle = "#FFF";
        ctx3.font = `bold ${Math.max(12, cw * 0.12)}px Arial`;
        ctx3.fillText(cfg.name, x + cw / 2, y + ch * 0.6);
        ctx3.fillStyle = "#CCC";
        ctx3.font = `${Math.max(10, cw * 0.09)}px Arial`;
        ctx3.fillText(cfg.desc, x + cw / 2, y + ch * 0.78);
      ...
      if (maxScroll > 0) {
        ...
        ctx3.beginPath();
        ctx3.roundRect(SCREEN_WIDTH - 8, barY, 4, barH, 2);
        ctx3.fill();
      }
      ctx3.textAlign = "left";
    }
    handleBottleTouch(x, y) {
      ...
      for (const b of this._bottleBtns) {
        if (x >= b.x && x <= b.x + b.w && adjustedY >= b.y && adjustedY <= b.y + b.h) return b;
      }
      return null;
    }
    ...
    _renderGameHUD(ctx3, d) {
      ...
      if (!isBigmap) {
        ...
        ctx3.fillRect(SCREEN_WIDTH - 90, 10, 35, 28);
        ctx3.fillStyle = "#333";
        ctx3.font = "14px Arial";
        ctx3.textAlign = "center";
        ctx3.fillText("\u{1F4D6}", SCREEN_WIDTH - 72, 29);
        ctx3.textAlign = "left";
        return;
      }
      const s = d.activeState();
      const p = SCREEN_WIDTH - 110, bw = 90, bh = 12;
      this._bar(ctx3, p + 20, SCREEN_HEIGHT - 35, bw, bh, s.playerHealth / s.maxPlayerHealth, "#f44336", "#ff5722");
      ctx3.fillStyle = "#FFF";
      ctx3.font = "bold 12px Arial";
      ctx3.textAlign = "right";
      ctx3.fillText("HP", p, SCREEN_HEIGHT - 25);
      ctx3.textAlign = "left";
    ...
    handleTouch(x, y, d) {
      if (x >= SCREEN_WIDTH - 90 && x <= SCREEN_WIDTH - 55 && y >= 10 && y <= 38) {
        d.isShowManual = !d.isShowManual;
        return "manual";
      }
      if (d.isShowManual) {
        const mw = SCREEN_WIDTH * 0.85, mh = SCREEN_HEIGHT * 0.7, mx = (SCREEN_WIDTH - mw) / 2, my = (SCREEN_HEIGHT - mh) / 2;
  ...
  var Main = class {
    ...
    _executeEnemyAttack(attacker) {
      ...
      if (attacker.type === 2) {
        if (attacker instanceof UFO) {
          this._goldShell(attacker);
        } else if (attacker instanceof Fjg) {
          this._antennaSlash(attacker);
        } else if (attacker instanceof Piao) {
          this._sonicRing(attacker);
        } else if (attacker instanceof Ding) {
          this._pierceBeam(attacker, targetX, targetY);
        } else {
    ...
    render() {
      ...
      if (d.selectedCategory === "mosquito_bigmap") {
        ...
        ctx2.fillRect(SCREEN_WIDTH - 28, 106, 22, 50);
        ctx2.save();
        ctx2.translate(SCREEN_WIDTH - 17, 131);
        ctx2.rotate(Math.PI / 2);
        ctx2.fillStyle = "#FFF";
        ctx2.font = "12px Arial";
        ctx2.textAlign = "center";
        ctx2.fillText("\u8FD4\u56DE", 0, 0);
        ctx2.restore();
    ...
    _renderEnemyAttacks(ctx3) {
      ...
      d.enemyAttacks.forEach((a) => {
        ...
        if (a.type === "pierceBeam") {
          ctx3.strokeStyle = "#E040FB";
          ctx3.lineWidth = a.width;
          ctx3.shadowColor = "#E040FB";
          ctx3.shadowBlur = 10;
          ctx3.beginPath();
          ctx3.moveTo(a.x, a.y);
          ctx3.lineTo(a.x + Math.cos(a.angle) * a.length, a.y + Math.sin(a.angle) * a.length);
          ctx3.stroke();
        }
        ctx3.restore();
      });
